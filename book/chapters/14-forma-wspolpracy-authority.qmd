...
W komunikacji operacyjnej celniejsze jest domyślne dzielenie się **linkami** do
kanonicznych obiektów (`ticket`, `artifact`, `log`, `report`) zamiast wysyłania
załączonych dokumentów. Ten sam celowy stan pracy ma wtedy jedną wersję źródła,
której wszyscy używają; każda decyzja i komentarz odwołuje się do tego samego
punktu prawdy. To ogranicza pracę na kopiach, zmniejsza ryzyko „różnicujących
wersji” dokumentów i skraca czas eskalacji, bo zamiast przesyłać treści, udostępnia
się już gotową ścieżkę wglądu.

Przykładowy wzorzec komunikacji operacyjnej:
...
Tożsamość informacji pozostaje w 1 miejscu, a komentarze i decyzje dokumentują
odwołanie do tego samego dowodu procesu.
